...
def read_bagofwords_dat(myfile, numofemails=10000):
    bagofwords = nmpy.fromfile(myfile, dtype=nmpy.uint8, count=-1, sep="")
    bagofwords = nmpy.reshape(bagofwords,(numofemails,-1))
    return bagofwords
...
categories = ['Spam', 'Not Spam']    # for case categories == None
...
y_train, y_test = trainTarget, testTarget
print("Extracting features from the training dataset using a sparse vectorizer")
t0 = time()
if opts.use_hashing:
    vectorizer = HashingVectorizer(stop_words='english', non_negative=True,
                                   n_features=opts.n_features)
    X_train = vectorizer.transform(dataTrain)
else:
